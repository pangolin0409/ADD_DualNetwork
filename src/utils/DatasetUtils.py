...
import torch
import torchaudio
from src.utils import TorchTimeStretch as TorchTimeStretch
...
class WaveTimeStretch(nn.Module):
    def __init__(self, max_ratio, min_ratio, sample_rate=16000, n_fft=0, audio_len=64600):
    ...
    def time_strech(self, audio, max_ratio, min_ratio, n_fft):
        ...
        output = TorchTimeStretch.time_stretch(input=audio, stretch=stretch_ratio, sample_rate=self.sample_rate, n_fft=n_fft)
        return output

    def forward(self, audio, max_ratio=None, min_ratio=None, n_fft=None):
        max_ratio = max_ratio or self.max_ratio
...
class AddEnvironmentalNoise(nn.Module):
    ...
    def load_noise_and_power(self, noise_path):
        ...
        noise_tensor = torchaudio.functional.resample(noise_tensor, noise_sr, self.sample_rate)
    ...
    def add_environmental_noise(self, audio, max_snr_db, min_snr_db):
        ...
        noise = self.noise_tensor.to(audio.device)  # ✅ 延遲搬上 GPU
        noise_power = torch.mean(noise ** 2, dim=-1, keepdim=True)

        noise_power_needed = signal_power / snr
        noise_power_factor = noise_power_needed / noise_power
        noise = noise * torch.sqrt(noise_power_factor)

        if self.add_before_audio_len is not None:
            tmp_zeros = torch.zeros_like(audio[:, :self.add_before_audio_len]) if audio.ndim == 2 else torch.zeros_like(audio[:self.add_before_audio_len])
    ...
    def forward(self, audio, max_snr_db=None, min_snr_db=None):
        ...
        if self.noise_category is None and self.noise_path is None:
            noise_category = random.choice(list(self.noise_dict.keys()))
            noise, noise_power = self.noise_dict[noise_category]
            self.noise_tensor = noise
            self.noise_tensor_power = noise_power
...
noise_dataset_path = os.path.join("src", "static", "audio")
manipulations = {
