...
class LayerExpert(nn.Module):
    def __init__(self, input_dim=1024, expert_dim=128, pooling_mode="avg"):
        """
        Args:
            input_dim (int): 該層輸出每個 time step 的維度
            expert_dim (int): 專家最終輸出維度
            pooling_mode (str): 池化方式，"avg" 為平均池化，"attn" 可擴展為 attention pooling
        """
        super().__init__()
        self.pooling_mode = pooling_mode
        # 這裡採用平均池化（若有需求可換成自定義 Attention Pooling）
        # 此外，也可在池化後加一層 LayerNorm 來穩定特徵
    ...
    def forward(self, x):
        ...
        if self.pooling_mode == "avg":
            ...
            # 這裡可以添加其他池化方式（如 attention pooling）
            pooled = x.mean(dim=1)
        out = self.proj(pooled)    # [B, expert_dim]
        return out

###############################################
# FixedMoE: 每個選定層分配一個專家，並融合專家輸出
###############################################
class FixedMoE(nn.Module):
    def __init__(self, layer_ids=[9, 10, 11, 12], input_dim=1024, expert_dim=128, 
                 num_classes=2, fusion_mode="avg"):
        """
        Args:
            layer_ids (list): 選用的層標識，如 [10, 11, 12, 13]
            input_dim (int): 每層輸出維度（例如 1024）
            expert_dim (int): 每個專家的輸出維度（例如 128）
            num_classes (int): 分類任務的類別數（例如 spoof/bonafide）
            fusion_mode (str): 融合方式： "concat" 或 "avg"
        """
        super().__init__()
        ...
        # 為每一個選定的層建立一個專家
        self.experts = nn.ModuleList([
            LayerExpert(input_dim=input_dim, expert_dim=expert_dim, pooling_mode="avg")
            for _ in layer_ids
        ])
        if fusion_mode == "concat":
            fusion_input_dim = expert_dim * len(layer_ids)
        elif fusion_mode == "avg":
            fusion_input_dim = expert_dim  # 平均融合後，維度回到 expert_dim
        else:
            raise ValueError("fusion_mode must be 'concat' or 'avg'")
        # 融合後送入 classifier 之前，可做一層線性映射和正規化
    ...
    def forward(self, hidden_states_dict):
        """
        Args:
            hidden_states_dict (dict): 鍵為層標識（如字符串"10", "11", ...），值為該層的 hidden states，[B, T, D]
        Returns:
            logits: [B, num_classes]
            expert_outputs: [B, fusion_input_dim] 融合前的專家輸出，可用於後續分析
        """
        expert_outputs = []
        for layer_id in self.layer_ids:
            key = layer_id
            if key not in hidden_states_dict:
                raise ValueError(f"Hidden states for layer {key} not found!")
            # 每個專家對應的輸入： [B, T, D]
            x = hidden_states_dict[key]
            out = self.experts[self.layer_ids.index(layer_id)](x)  # [B, expert_dim]
            expert_outputs.append(out)
        
        if self.fusion_mode == "concat":
            fused = torch.cat(expert_outputs, dim=-1)  # [B, expert_dim * n]
        elif self.fusion_mode == "avg":
            # 平均融合各專家輸出
            stacked = torch.stack(expert_outputs, dim=0)  # [n, B, expert_dim]
            fused = stacked.mean(dim=0)  # [B, expert_dim]
        fused_proj = self.fusion_proj(fused)  # [B, 256]
        logits = self.classifier(fused_proj)  # [B, num_classes]
        return logits, fused  # 返回 logits 與未融合之前或融合之後的專家輸出（用於分析）

###############################################
# Detector: 將 wav2vec2-large 的多層 hidden states 輸入 FixedMoE
###############################################
class Detector(nn.Module):
    def __init__(self, encoder_dim=1024, expert_dim=128, num_classes=2, 
                 layer_ids=[9, 10, 11, 12], fusion_mode="avg"):
        super().__init__()
        # FixedMoE 處理多層專家融合與分類
        self.moe = FixedMoE(layer_ids=layer_ids, input_dim=encoder_dim, 
                             expert_dim=expert_dim, num_classes=num_classes, 
                             fusion_mode=fusion_mode)
    
    def forward(self, wav2vec_hidden_states):
        """
        Args:
            wav2vec_hidden_states (tuple): Wav2Vec2 模型輸出的 hidden_states，
                長度 25 的 tuple，第一項為 CNN encoder 輸出，後續 24 為 Transformer 層
                我們將根據指定的 layer_ids 取出對應層，並組成一個字典，
                鍵為字符串形式，如 {"10": tensor, "11": tensor, ...}
        Returns:
            logits: [B, num_classes]
            moe_feature: 專家融合後的特徵（可用於後續分析）
        """
        # 構建 hidden_states 的字典
        # 注意：假設 layer index 0 為 CNN encoder，1~24 為 Transformer 層
        # 如果你希望使用 Transformer 層 10, 11, 12, 13，則字典鍵為 "10", "11", "12", "13"
        hidden_dict = {}
        for layer_id in self.moe.layer_ids:
            # 此處 hidden_states tuple 的索引直接對應你希望用的層
            hidden_dict[layer_id] = wav2vec_hidden_states[layer_id]
        logits, moe_feature = self.moe(hidden_dict)
        return logits, moe_feature